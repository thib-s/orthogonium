...
class OrthoConv(nn.Conv2d):
    ...
    def forward(self, X):
        ...
        return super(OrthoConv, self).forward(X)


class OrthoConvTranspose(nn.ConvTranspose2d):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        if dilation != 1:
            raise RuntimeError("dilation not supported")
        super(OrthoConvTranspose, self).__init__(
            in_channels,
            out_channels,
            kernel_size,
            stride,
    ...
    def forward(self, X):
        self._input_shape = X.shape[2:]
        return super(OrthoConvTranspose, self).forward(X)
